...
We currently use files to represent streams. 
In the future, 
we intend to provide Wayeb with the capability to consume events from Kafka topics. 
See [stream](src/main/scala/stream) package for all relevant code.

### Streams from files

Event streams should currently be consumed from files. 
Each event typically has a timestamp, and event type and a unique id.
Any other attributes may also be present as a Map of attribute names to values
(see [stream.GenericEvent](src/main/scala/stream/GenericEvent.scala)).
...
(see [stream.source.GenericCSVStreamSource](src/main/scala/stream/source/GenericCSVStreamSource.scala)).
...
see [stream.domain.maritime.MaritimeStreamSourceCSV](src/main/scala/stream/domain/maritime/MaritimeStreamSourceCSV.scala).
If you do so,
then you need to also modify the function stream.StreamFactory.getDomainStreamSource
in [stream.StreamFactory](src/main/scala/stream/StreamFactory.scala) 
...
(see [stream.source.JsonStreamSource](src/main/scala/stream/source/JsonStreamSource.scala)).
...
Again, prefix notation is used.
 ...
 then `|(P,-Q)` is a complex sentence (`P` or not `Q`).
...
(see [fsm.symbolic.sfa.logic.predicates.IsEventTypePredicate](src/main/scala/fsm/symbolic/sfa/logic/predicates/IsEventTypePredicate.scala)).

Note that for each predicate in a pattern, 
there must exist an implementation with the same name under
[fsm.symbolic.sfa.logic.predicates](src/main/scala/fsm/symbolic/sfa/logic/predicates).
