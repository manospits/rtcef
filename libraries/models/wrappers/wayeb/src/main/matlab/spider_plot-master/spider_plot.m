...
% Polar points
radius = [0; axes_limit];
%radius_increments = axes_limit/num_of_points;
%radius = 0:radius_increments:axes_limit;
theta = 0:polar_increments:2*pi;

[X,Y] = meshgrid(theta,radius);

% Convert polar to cartesian coordinates
[x_axes, y_axes] = pol2cart(X, Y);
...
%radius = linspace(0,axes_limit,size(theta,2));

[X,Y] = meshgrid(theta,radius);
% Convert polar to cartesian coordinates
[x_isocurves, y_isocurves] = pol2cart(X, Y);
...
%     thisAxisIncrement = axis_increment(hh);
%     %if axis_increment(hh) == 0 
%     %    thisAxisIncrement = 0.01;
%     %else
%     %    thisAxisIncrement = axis_increment(hh);
%     %end
%     % Shifted min value
%     shifted_min_value = min_values(hh)-thisAxisIncrement; %axis_increment(hh);
%     
%     % Axis label for each row
%     row_axis_labels = (shifted_min_value:thisAxisIncrement:max_values(hh))';
%     
