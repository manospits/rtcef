function speedMax(maxSpreads, thresholds, ordersSDFA, ordersSPSA, resultsSDFA, resultsSPSA, columnSDFA, columnSPSA, metric, titlePrefix)
    ...
    for spread=maxSpreads
        for threshold=thresholds
            ...
            %plot(ordersSDFA(validIndices),currentScores(validIndices),'--*','LineWidth',5.0,'MarkerSize',15);
            plot(ordersSDFA,currentScores,'--*','LineWidth',5.0,'MarkerSize',15);

            currentScores = scoresSPSA(r,:);
            %validIndices = find(currentScores ~= -1);
            hold on;
            %plot(ordersSPSA(validIndices),currentScores(validIndices),'-.o','LineWidth',5.0,'MarkerSize',15);
            ...
            title(figureTitle);
            set(gcf,'Color','w');
            pdfTitle = strcat(titlePrefix, 'SpeedMax',metric{1,1}, 'Spread', num2str(spread), 'Threshold', num2str(100*threshold), '.pdf')
            export_fig(pdfTitle);
        end
    end
end
