function meetFixed(maxSpreads, distances, thresholds, orders, results, column, metric, titlePrefix)
    ...
    for spread=maxSpreads
        ...
        for threshold=thresholds
            ...
            for distance=distances
                ...
                for order=orders
                    j = j + 1;
                    index = find(results(:,1)==order & results(:,2)==spread & results(:,3)==distance & results(:,4)==threshold);
    ...
    for spread=maxSpreads
        ...
        for threshold=thresholds
            ...
            title(figureTitle);
            set(gcf,'Color','w');
            pdfTitle = strcat(titlePrefix,'MeetFixed',metric{1,1},'Spread',num2str(spread),'Threshold',num2str((100*threshold)),'.pdf')
            export_fig(pdfTitle);
        end
    end
