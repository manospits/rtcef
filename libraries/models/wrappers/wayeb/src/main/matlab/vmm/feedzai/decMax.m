function decMax(maxSpreads, thresholds, ordersSDFA, ordersSPSA, resultsSDFA, resultsSPSA, columnSDFA, columnSPSA, metric, titlePrefix)
    ...
    for spread=maxSpreads
        for threshold=thresholds
            ...
            title(figureTitle);
            set(gcf,'Color','w');
            pdfTitle = strcat(titlePrefix, 'IncMax',metric{1,1}, 'Spread', num2str(spread), 'Threshold', num2str(100*threshold), '.pdf')
            export_fig(pdfTitle);
        end
    end
end
