function trainTimes = gatherTrainTimes(minDistances, maxDistances, orders, maxSpread, results)
    ...
    for g=1:groupsNo
        ...
        for s=1:stacksPerGroup
            ...
            extraTime = mean(results(index,20));
            modelTime = mean(results(index,21));
            wtTime = mean(results(index,22));
            predTime = mean(results(index,23));
