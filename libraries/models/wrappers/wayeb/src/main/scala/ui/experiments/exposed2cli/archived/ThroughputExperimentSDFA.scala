...
import workflow.provider.{FSMProvider, ForecasterProvider, SDFAProvider}
import workflow.task.engineTask.ERFTask

object ThroughputExperimentSDFA extends LazyLogging {
  def RunExperiment(
      patternName: String,
      domain: String,
      resultsDir: String,
    ...
    while (iteration < iterations) {
      ...
      val sdfaifn: String = resultsDir + "/SDFAI_m" + order + ".sdfai"
      logger.info("Creating FSM provider from " + sdfaifn)
      val sdfap = SDFAProvider(SDFASourceSerialized(sdfaifn))
      val fsmp = FSMProvider(sdfap)

      val wtifn: String = resultsDir + "/SDFA_WTI_m" + order + "_h" + horizon + "_t" + threshold + "_s" + maxSpread + "_m" + spreadMethod + ".wti"
