...
class ERFEngine  (
  ...
  var fsmProvider = givenfsmProvider
  val singlePartitionVal = ConfigUtils.singlePartitionVal
  var predList = predProvider.provide()
  var fsmList = fsmProvider.provide().zip(predList).map(x => {
    if (distance._1 != -1.0 & distance._1 < 1.0) {
      if (x._1.remainingPercentage.isEmpty) x._1.estimateRemainingPercentage
      x._1
    } else x._1
  })
  var runPool: RunPool = RunPool(fsmList, expirationDeadline, distance, show)
  var md = new MatchDump()
  var predFactory: ForecasterRunFactory = ForecasterRunFactory(predList, collectStats, finalsEnabled)
  var predictorRuns: Map[Int, List[ForecasterRun]] = fsmList.map(fsm => fsm.getId).
    map(id => id -> List.empty[ForecasterRun]).toMap

  var matchesNo = 0
  var execTime: Long = 0
  var lastCollectTime: Long = 0
  var streamSize: Int = 0
  var profiler: WtProfiler = WtProfiler()
  ...
  /**
    ...
    */
  def processEvent(event: GenericEvent): Unit = {
