...
class SPSTProvider private (
  ...
  override def provide(): List[SPSTInterface] = {
    spstSource match {
      ...
      case x: SPSTSourceFromSDFA => sdfa2spst(x.sdfap, x.trainStreamSource, x.order, x.pMin, x.alpha, x.gamma, x.r)
  ...
  private def sre2spst(
    ...
    val spsti = sdfaList.indices.map(f => {
      ...
      val pstp = PSTProvider(PSTSourceLearner(sdfap, streamSource, ordersList(f), pMin, alpha, gammaMin, r))
      val pst = pstp.provide().head
      val spsti = SPSTInterface(pst._1, sdfaList(f), pst._2, ordersList(f), f+1, attrsList(f))
      spsti
    }).toList
    spsti
  }
  ...
  /**
    * Learns SPSA from SDFA and a training stream.
    *
    * @param sdfap The provider for the SDFA.
    * @param trainStreamSource The source for the training stream  (same for all SPST).
    * @param order The order of the SPST (same for all SPST).
    * @param pMin This is the symbol threshold. Symbols with lower probability are discarded (same for all SPST).
    * @param alpha Used to calculate the conditional threshold = (1 + alpha) * gammaMin. The conditional on the expanded
    *              context must be greater than this threshold (same for all SPST).
    * @param gammaMin Used to calculate the conditional threshold = (1 + alpha) * gammaMin. The conditional on the
  ...
  private def sdfa2spst(
    ...
    sdfais.map(sdfai => sdfa2spst(sdfai, trainStreamSource, order, pMin, alpha, gammaMin, r, sdfai.id))
  }
  ...
  /**
    ...
    * @param trainStreamSource The source for the training stream.
  ...
  private def sdfa2spst(
    ...
    val sdfap = SDFAProvider(SDFASourceDirect(List(sdfai.sdfa), List(sdfai.partitionAttribute)))
    val pstp = PSTProvider(PSTSourceLearner(sdfap, trainStreamSource, order, pMin, alpha, gammaMin, r))
    val pst = pstp.provide().head
    val spsti = SPSTInterface(pst._1, sdfai.sdfa, pst._2, order, id, sdfai.partitionAttribute)
    spsti
  }
