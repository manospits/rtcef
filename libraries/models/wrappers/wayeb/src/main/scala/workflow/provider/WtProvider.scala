...
import workflow.provider.source.wt.{WtSource, WtSourceDirect, WtSourceMatrix, WtSourceRT, WtSourceSPST, WtSourceSerialized}
...
object WtProvider {
  ...
  def apply(wtSource: WtSourceSerialized): WtProvider =
    new WtProvider(wtSource, List(new FileExistsCondition(wtSource.fn)))
}
...
class WtProvider private (
  ...
  override def provide(): List[Map[Int, WtDistribution]] = {
    wtSource match {
      ...
      case x: WtSourceSerialized => deserializeFile(x.fn)
  ...
  private def deserializeFile(fn: String): List[Map[Int, WtDistribution]] = {
    val oisWTS = new ObjectInputStream(new FileInputStream(fn))
    val wtds = oisWTS.readObject.asInstanceOf[List[Map[Int, WtDistribution]]]
    oisWTS.close()
    wtds
  }
}
