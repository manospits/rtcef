...
class WtProfiler() extends ProfilerInterface with LazyLogging {
  ...
  private var statsEstimators: Map[Int, StatsEstimator] = Map.empty
   ...
   * @return a list with the TP, TN, FP, FN
   */
  def getTPTNFPFN(forPattern: Int):List[Double] = {
    val tp = getStatFor("tp", forPattern).toDouble
    val tn = getStatFor("tn", forPattern).toDouble
    val fp = getStatFor("fp", forPattern).toDouble
    val fn = getStatFor("fn", forPattern).toDouble
