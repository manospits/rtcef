...
object VMMUtils extends LazyLogging {
  ...
  /**
    * Learns a SPSA from a formula and a training stream.
    * Same as model.vmm.VMMUtils#learnSPSAFromSingleFormula(scala.Tuple3, scala.collection.immutable.Set, scala.collection.immutable.Set, scala.Enumeration.Value, stream.source.StreamSource, int, model.vmm.Isomorphism),
  ...
  private def learnPredictionSuffixTree(
    ...
    logger.debug("Learning prediction suffix tree from iso")
    val cst = learnCounterSuffixTree(streamSource, maxOrder, iso, partitionAttribute)
    val pstl = PSTLearner(iso.getSymbols.toSet, maxOrder, pMin, alpha, gammaMin, r)
    val pst = pstl.learnVariant(cst, withMissing = true)
    logger.debug("Done with learning prediction suffix tree")
    pst
  }
  ...
  def learnPredictionSuffixTree(
    ...
    val cst = learnCounterSuffixTree(streamSource, maxOrder, iso, partitionAttribute)
    val pstl = PSTLearner(iso.getSymbols.toSet, maxOrder, pMin, alpha, gammaMin, r)
    val pst = pstl.learnVariant(cst, withMissing = false)
    logger.debug("Done with learning prediction suffix tree")
    (pst, iso)
  }
  ...
  /**
    ...
    * @param iso The isomorphism providing the symbols.
    * @param partitionAttribute The partition attribute of the pattern from which the isomorphism was derived.
  ...
  */
  ...
  /**
    ...
    */
  private def createIsomorphism(sdfa: SDFA): Isomorphism = {
