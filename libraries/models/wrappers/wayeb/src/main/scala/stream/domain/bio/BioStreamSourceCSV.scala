...
object BioStreamSourceCSV {
  /**
    * Constructor for stream source for biological simulations.
    * @param filename The path to the file.
    * @return The stream source.
    */
  def apply(filename: String): BioStreamSourceCSV = new BioStreamSourceCSV(filename)
}

/**
  * Stream for biological simulations. All simulations in a single csv file, separated by RESET events.
  *
  * @param filename The path to the file.
  */
class BioStreamSourceCSV(filename: String) extends CSVStreamSource(filename) with LazyLogging {
