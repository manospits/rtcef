...
import model.vmm.{Isomorphism, Symbol, SymbolWord}
...
class SPSTInterface(
  ...
  private val sdfaStates = sdfa.states.keySet

  // now combine PST labels and SDFA states to get all the virtual states
  private val labelsXStates = labels.flatMap(l => sdfaStates.map(s => (l, s))).toList
  ...
  //isolate the start vistual states
  ...
  override def getNextState(
    ...
    val sdfaNextState = sdfa.getDelta(sdfaCurrentState, event).head
    ...
    val labelXState = (pstState.label, sdfaNextState)
